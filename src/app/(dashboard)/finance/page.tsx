    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <ActionButton onClick={() => { setModalType("debit"); setIsModalOpen(true); }} icon={Minus} label="Record Spend" color="text-primary" bg="bg-primary/10" border="border-primary/20" />
          ...
          <button onClick={clearLedger} className="p-3 bg-secondary/10 text-muted-foreground hover:text-red-500 rounded-2xl transition-all"><RotateCcw className="h-5 w-5" /></button>
        </div>
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <FinanceStatCard title="Ledger Balance" value={formatCurrency(totalBalance)} icon={Wallet} color="text-secondary" bg="bg-secondary/10" description="Total overall balance" />
              ...
              {activeTab === "history" && (
                <motion.div key="history" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-4">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <span className="text-[10px] font-bold bg-secondary/20 px-3 py-1 rounded-full uppercase tracking-widest text-muted-foreground">{new Date().toLocaleString('default', { month: 'long' })} Records</span>
                  </div>
              ...
              <form onSubmit={handleAdd} className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 space-y-2">
                    ...
                    <input type="number" step="0.01" required value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" className="w-full text-3xl font-heading font-bold px-4 py-4 rounded-2xl bg-background border border-monk-rose/20 focus:border-primary/50 focus:outline-none transition-all" />
                  </div>
                  {modalType === "debt" && (
                ...
                <button type="submit" className="w-full py-5 rounded-2xl bg-foreground text-background font-bold text-lg shadow-xl hover:scale-[1.02] active:scale-95 transition-all">Save to Ledger</button>
              </form>
            </motion.div>
          </div>
        )}
