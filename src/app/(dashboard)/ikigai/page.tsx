...
const calculateClarityScore = (entry: Partial<IkigaiEntry>) => {
  // Logic to calculate dynamic clarity score
  ...
  // Mix in some external stats from localStorage
        ...
        <div className="relative z-10 space-y-10">
          ...
          <div className="space-y-2">
            ...
            <div className="h-1 w-full bg-secondary rounded-full overflow-hidden">
               <motion.div 
                 initial={{ width: 0 }}
                 animate={{ width: `${((step + 1) / REEVALUATION_QUESTIONS.length) * 100}%` }}
          ...
          >
            <h3 className="text-3xl font-heading font-bold text-foreground leading-tight">
              {REEVALUATION_QUESTIONS[step].question}
            </h3>
            <textarea 
              autoFocus
              value={answers[step]}
          ...
          <div className="flex justify-between items-center pt-4">
             <button 
              ...
              disabled={step === 0}
              className="text-xs font-black uppercase tracking-widest text-text-secondary disabled:opacity-0 hover:text-foreground transition-all"
             >
              Previous
             </button>
             <button 
        ...
        {data.map((point, i) => (
          ...
          >
            ...
            <div className="absolute top-6 left-1/2 -translate-x-1/2 bg-card border border-border px-3 py-1.5 rounded-lg shadow-xl whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all pointer-events-none z-20">
               ...
               <p className="text-[8px] font-bold text-text-secondary uppercase">
                 {point.skill > 80 ? "Close to mastery here" : "Enjoyable, skill growing"}
               </p>
            </div>
          </motion.div>
        ))}
...
export default function IkigaiPage() {
    ...
    if (saved) {
      ...
      if (legacy) {
        ...
        const initialEntry: IkigaiEntry = {
          ...
          reflections: [],
          alignmentData: [
            { passion: 70, skill: 50, label: "Initial Focus" }
          ]
        };
  ...
  const handleCompleteReevaluation = (newEntry: IkigaiEntry) => {
    const updatedEvolution = {
      entries: [newEntry, ...evolution.entries],
    ...
    window.dispatchEvent(new Event("streak_updated")); // Trigger analytics refresh
  };

  const recommendations = useMemo(() => {
    if (!activeEntry) return [];
    return generateRecommendations(activeEntry);
  }, [activeEntry]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 pb-24 px-4 md:px-8 animate-in fade-in duration-1000">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-8 space-y-8 text-center lg:text-left">
            <div className="space-y-4">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-heading font-black tracking-tighter leading-tight text-text-primary">
                {activeEntry ? `"${activeEntry.purposeStatement}"` : "Define Your Direction."}
              </h1>
              <p className="text-lg md:text-xl text-text-secondary font-soft italic">
                "Your purpose becomes clearer through action."
              </p>
            </div>
            ...
            <div className="pt-4">
               <button 
                ...
                className="px-10 py-5 bg-zinc-900 dark:bg-white text-white dark:text-black font-black text-[10px] uppercase tracking-[0.3em] rounded-[24px] hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-primary/20 flex items-center gap-3 mx-auto lg:mx-0"
               >
                 <RotateCcw className="h-4 w-4" /> {activeEntry ? "Re-evaluate My Ikigai" : "Start First Reflection"}
               </button>
            </div>
          </div>
             ...
             <div className="relative h-48 w-48 flex items-center justify-center">
                <svg viewBox="0 0 100 100" className="h-full w-full -rotate-90 drop-shadow-[0_0_20px_rgba(246,193,204,0.3)]">
                   ...
                   <motion.circle 
                     cx="50" cy="50" r="45" 
                     className={cn(
                       "fill-none stroke-[6] stroke-linecap-round transition-colors duration-1000",
                       (activeEntry?.clarityScore || 0) > 70 ? "stroke-primary" : "stroke-text-secondary/40"
                     )}
                     strokeDasharray="283"
                     initial={{ strokeDashoffset: 283 }}
                     animate={{ strokeDashoffset: 283 - (283 * (activeEntry?.clarityScore || 0)) / 100 }}
                     transition={{ duration: 2, ease: "easeOut" }}
                   />
                </svg>
                <div className="absolute flex flex-col items-center text-center">
                   <span className="text-5xl font-heading font-black text-text-primary tracking-tighter">{activeEntry?.clarityScore || 0}</span>
                   <span className="text-[8px] font-black text-text-secondary uppercase tracking-widest mt-1">Clarity Score</span>
                </div>
             </div>
             <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest max-w-[140px] text-center leading-relaxed">
               Clarity grows when actions align with values.
             </p>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        {/* Left Column: Alignment & Recommendations */}
        <div className="lg:col-span-7 space-y-10">
           {/* 3. Alignment Graph */}
           <section className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-heading font-black tracking-tighter uppercase italic flex items-center gap-3">
                  <TrendingUp className="h-6 w-6 text-primary" /> Passion vs Skill Alignment
                </h3>
              </div>
           ...
           <section className="space-y-6">
              <h3 className="text-xl font-heading font-black tracking-tighter uppercase italic flex items-center gap-3 text-text-primary">
                <Sparkles className="h-6 w-6 text-accent" /> Next Small Steps
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 {recommendations.map((rec, i) => (
                   <div key={i} className="monk-card p-6 border-2 border-border/50 hover:border-primary/30 transition-all space-y-4 group">
                      ...
                      <div className="space-y-1">
                        <h4 className="font-bold text-sm text-text-primary uppercase tracking-tight">{rec.title}</h4>
                        <p className="text-xs text-text-secondary font-soft leading-relaxed">{rec.desc}</p>
                      </div>
                   </div>
                 ))}
              </div>
        ...
        <div className="lg:col-span-5 space-y-6">
           <h3 className="text-xl font-heading font-black tracking-tighter uppercase italic flex items-center gap-3 text-text-primary">
             <History className="h-6 w-6 text-text-secondary" /> Evolution Timeline
           </h3>
           ...
           <div className="space-y-4 relative">
             ...
             {evolution.entries.map((entry, i) => (
               <motion.div 
                 ...
                 transition={{ delay: i * 0.1 }}
                 className={cn(
                   "relative pl-14 pr-6 py-6 rounded-[32px] border-2 transition-all group",
                   i === 0 ? "bg-card border-primary/20 shadow-xl" : "bg-background border-border/50 opacity-60 hover:opacity-100"
                 )}
               >
                 <div className={cn(
                   "absolute left-5 top-8 h-4 w-4 rounded-full border-4 border-background transition-colors",
                   i === 0 ? "bg-primary" : "bg-border group-hover:bg-text-secondary"
                 )} />
                 ...
                 <div className="space-y-3">
                    <div className="flex items-center justify-between">
                       ...
                       <span className="text-xs font-black text-primary">{entry.clarityScore}% Clarity</span>
                    </div>
                    <p className="text-sm font-heading font-bold text-text-primary leading-snug">
                       {i === 0 ? "Alignment Increasing" : i === 1 ? "Direction Forming" : "Exploring Options"}
                    </p>
                    <p className="text-xs text-text-secondary font-soft italic line-clamp-2">
                       {entry.purposeStatement}
                    </p>
                 </div>
               </motion.div>
             ))}
             ...
             {evolution.entries.length === 0 && (
               <div className="py-20 text-center opacity-30 space-y-4">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.3em]">No evolution recorded yet</p>
               </div>
             )}
           </div>
        </div>
      ...
      {/* Reflection Modal */}
      <ReflectionModal 
        isOpen={showReevaluation} 
        onClose={() => setShowReevaluation(false)} 
