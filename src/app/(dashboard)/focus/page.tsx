...
export default function FocusPage() {
  ...
  const deleteLog = (id: string) => {
    ...
    if (user) {
      syncManager.save('focus_sessions', 'DELETE', { id });
    }
  };
  ...
  const clearHistory = () => {
    ...
    if (user) {
      currentHistory.forEach(s => {
        syncManager.save('focus_sessions', 'DELETE', { id: s.id });
      });
    }
  };
  ...
  useEffect(() => {
    ...
    if (isActive && timeLeft > 0) {
      ...
      if (user) {
        syncManager.save('focus_sessions', 'INSERT', {
          id: newSession.id,
          user_id: user.id,
  ...
  }, [isActive, timeLeft, mode, history, targetDuration, user]);
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-end">
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="relative w-full max-w-md h-full bg-card border-l border-border shadow-2xl flex flex-col p-8 md:p-10">
              <div className="flex items-center justify-between mb-10">
                <div className="flex items-center gap-3"><div className="h-11 w-11 md:h-10 md:w-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary"><History className="h-5 w-5" /></div><h2 className="text-2xl font-heading font-black tracking-tighter uppercase italic">Focus Logs</h2></div>
                <button onClick={() => setShowLogs(false)} className="p-3 md:p-2 hover:bg-secondary rounded-full transition-all text-text-secondary"><X /></button>
              </div>
                ...
                {history.length > 0 ? history.map((session) => (
                  <div key={session.id} className="flex items-center justify-between p-5 rounded-[24px] bg-secondary/30 border border-border group transition-all hover:border-primary/20">
                    <div className="flex items-center gap-4">
                      <div className="h-11 w-11 md:h-10 md:w-10 rounded-xl flex items-center justify-center bg-card shadow-sm text-primary"><Zap className="h-5 w-5" /></div>
                      <div>
      ...
      <AnimatePresence>
        {showSettings && (
          <div className="fixed inset-0 z-[100] flex items-center justify-end">
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="relative w-full max-w-md h-full bg-card border-l border-border shadow-2xl flex flex-col p-8 md:p-10">
              <div className="flex items-center justify-between mb-10">
                <div className="flex items-center gap-3"><div className="h-11 w-11 md:h-10 md:w-10 rounded-xl bg-accent/10 flex items-center justify-center text-accent"><Settings2 className="h-5 w-5" /></div><h2 className="text-2xl font-heading font-black tracking-tighter uppercase italic">Timer Settings</h2></div>
                <button onClick={() => setShowSettings(false)} className="p-3 md:p-2 hover:bg-secondary rounded-full transition-all text-text-secondary"><X /></button>
              </div>
