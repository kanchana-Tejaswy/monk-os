...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      alert("Google Authentication failed: Supabase environment variables are missing. Please configure NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md space-y-12 animate-in fade-in zoom-in duration-1000 flex flex-col items-center">
        ...
        {/* Auth Box */}
        <div className="w-full monk-card p-8 space-y-6">
          <button 
            onClick={handleGoogleLogin}
            className="w-full flex items-center justify-center gap-3 bg-foreground text-background font-bold py-4 rounded-xl hover:scale-[1.02] transition-all shadow-xl active:scale-95"
          >
